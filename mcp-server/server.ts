...
async function fetchModels(): Promise<ModelSummary[]> {
  ...
  if (!res.ok) throw new Error(`Jart-URA /v1/models: ${res.status}`);
  const body = await res.json();
  return body.data as ModelSummary[];
}

async function fetchHealth(): Promise<HealthStatus> {
  const res = await fetch(`${JART_URA_BASE}/health`);
  if (!res.ok) throw new Error(`Jart-URA /health: ${res.status}`);
...
export function createServer(): McpServer {
  ...
  registerAppTool(
    ...
    "get-dashboard",
    {
      ...
      description:
        "Returns the full model registry and health status, and opens an interactive dashboard UI for model management.",
      inputSchema: {},
  ...
  registerAppTool(
    ...
    async (args: Record<string, unknown>): Promise<CallToolResult> => {
      ...
      return {
        ...
        structuredContent: model as unknown as { [x: string]: unknown },
      };
    },
  );
  ...
  registerAppTool(
    ...
    "poll-model-status",
    {
      ...
      description:
        "Returns current model status and health for live dashboard updates. App-only.",
      inputSchema: {},
      outputSchema: {},
  ...
  registerAppResource(
    ...
    resourceUri,
    resourceUri,
    { mimeType: RESOURCE_MIME_TYPE, description: "Jart-URA Dashboard UI" },
